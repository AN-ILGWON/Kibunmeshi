...
-- 既存データのクリーンアップ (テーブルリセット順序)
-- 注意: users 테이블은 사용자 데이터 보존을 위해 제외함
SET FOREIGN_KEY_CHECKS = 0;
TRUNCATE TABLE recommendation_histories;
TRUNCATE TABLE foods;
TRUNCATE TABLE food_categories;
TRUNCATE TABLE emotions;
TRUNCATE TABLE food_advice_templates;
SET FOREIGN_KEY_CHECKS = 1;

-- ============================================================
-- 1. emotions（気分マスター）- 6種類
-- ============================================================
...
-- ============================================================
-- 2. food_categories（料理カテゴリマスター）- 6種類
-- ============================================================
...
-- ============================================================
-- 3. foods（料理データ - 36가지 감정 x 카테고리 조합 전수 수록）
-- ============================================================
...
('チーズタッカルビ', 1, 1, '甘辛く炒めた鶏肉と野菜に濃厚トロトロのチーズを絡めた華やかフュージョン料理。', 'タンパク質とカルシウム가 豊富で、エネルギー補給に最適です。', 'fun', 'cheese dakgalbi', 'https://images.unsplash.com/photo-1567620832903-9fc6debc209f?w=600&auto=format&fit=crop', 'common'),
...
('四川シビ辛麻婆豆腐', 3, 3, '花椒（ホアジャオ）の痺れる辛さと唐辛子がクセになる本格四川麻婆豆腐。', '花椒のサンショオール成分が脳をシャキッとリフレッシュ。', 'spicy', 'mapo tofu', 'https://images.unsplash.com/photo-1541696432-82c6da8ce7bf?w=600&auto=format&fit=crop', 'rare'),
...
('ファラフェル', 3, 6, 'ひよこ豆と香辛料を練り込んで揚げたスパイシーな中東のコロッケ。', '豆のタンパク質とスパイス가 イライラを解消します。', 'reset', 'falafel', 'https://images.unsplash.com/photo-1593001874117-c99c800e3e7a?w=600&auto=format&fit=crop', 'common'),
...
(1, 'fun', '楽しい気分には{food_name}がぴったり！美味しく食べて笑顔を増やそう。', '즐거운 기분에는 {food_name}이 딱이에요! 맛있게 먹고 웃음을 늘려봐요.', TRUE),
(1, 'colorful', 'カラフルな気分には{food_name}！見た目も味も楽しんで。', '화려한 기분에는 {food_name}! 보기에도 맛에도 즐겨보세요.', TRUE),
...
(3, 'refresh', 'スッキリした味の{food_name}で気分転換！', '상쾌한 taste의 {food_name}으로 기분 전환!', TRUE),
...
SELECT CONCAT('初期データ投入完了: ', 
    (SELECT COUNT(*) FROM emotions), ' emotions, ',
    (SELECT COUNT(*) FROM food_categories), ' categories, ',
    (SELECT COUNT(*) FROM foods), ' foods'
) AS status;
